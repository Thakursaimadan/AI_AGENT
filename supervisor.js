...
async function handleInput(text) {
  try {
    ...
    if (pendingOperation && pendingOperation.componentIds && pendingOperation.componentIds.length > 1) {
      const selectedIndex = parseInt(text.trim()) - 1;
      if (selectedIndex >= 0 && selectedIndex < pendingOperation.componentIds.length) {
        const selectedComponentId = pendingOperation.componentIds[selectedIndex];
        ...
        const editorResult = await EditorAgent.invoke({
          messages: [
            ...
            ...conversation,
            new HumanMessage({ 
              content: `Update component with clientId: ${pendingOperation.clientId}, componentId: ${selectedComponentId}, updates: ${JSON.stringify(pendingOperation.updates)}` 
            })
          ]
        });
        
        pendingOperation = null; // Clear pending operation
        const lastAIMessage = editorResult.messages.filter(m => m._getType() === 'ai').pop();
    ...
    if (route === 'editor') {
      ...
      if (lastAIMessage && lastAIMessage.content.includes('need to clarify')) {
        ...
        return lastAIMessage ? lastAIMessage.content : "No response from editor agent.";
      }
    }
    ...
    if (route === 'clarify') {
      ...
      const lastAIMessage = clarifierResult.messages.filter(m => m._getType() === 'ai').pop();
      ...
      return lastAIMessage ? lastAIMessage.content : "No response from clarifier agent.";
    }
